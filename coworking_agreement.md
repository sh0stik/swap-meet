# Coworking Agreement

Talk through each section with your partner. Add notes on what you discussed and agreed upon in each section. At the bottom, type your names to sign off on your agreement.

## Accessibility Needs

Daria: At this time, no needs idenitfied.

Rahma: N/A

## Collaboration vs. Individual Work Expectations

Work flow: 

- Seperate branches for Wave 1 - 6
- Pull requests to merge feature branches into the main, author merges branch using below schedule.
- Consecutively, merge Wave 1 into Main, then Wave 2 into Main .. until Wave 6


Schedule:

1. Daria: Wave 1-2 (PR Merged by Tuesday 9:00 AM PST)

2. Rahma: Wave 3-4  (PR Merged by Tuesday 1:00 PM PST)

- Meet on Tuesday 1-3 PM to review Wave 1-4

3. Rahma: Wave 5 (PR Open by Wednesday 1:00 PM PST) 

- Meet on Wednesday 2-3 PM review Wave 5, Daria can close the PR for Wave 5

4. Daria: Wave 6 (PR Merged by Wednesday 5:00 AM PST ) 

- Meet on Thursday 2-3 PM to final touches, any optional code, submit on Learn. 


Refractor/Review code:
- Discussion was done after working Wave 1 - 6
- Review during coworking sessions.

## Learning Style

Daria: Learn when she is doing, and trying new approaches. When debugging, debugger first, then rubber ducking, outside resources including internet/Gen AI.

Rahma: 5-10 minutes to silently read and take notes on a piece of new information. For debugging, I like to have again taken 5-10 minutes to generate error message (Terminal Commands + Debugger) and Google/Gen AI. 

## Preferred Feedback Style

We agree to comments via Slack and open a new branch for PRs if a requirement is missed or code is broken. 

## One Team Communication Skill to Improve

Daria: Giving feedback.

Rahma: Giving feedback.

## Optional: Other agreements

N/A

## Signatures
Daria S. 
Date: 9/21/2026

Rahma M.
Date: 9/21/2026